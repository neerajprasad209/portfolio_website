  ...
  return <>
      ...
      <nav className={`fixed right-0 top-0 h-screen w-20 md:w-24 flex-col items-center bg-navy-dark border-l border-navy-light hidden lg:flex transition-all duration-300 ${scrolled ? 'border-opacity-50' : 'border-opacity-0'}`}>
        ...
        <div className="flex flex-col items-center justify-center h-full my-0 py-[26px]">
          ...
          <div className="h-40 w-[1px] bg-slate opacity-30 mt-8"></div>
        </div>
      </nav>
      ...
      <div className="fixed top-0 left-0 w-full z-50 lg:hidden">
        <div className={`p-4 flex justify-between items-center transition-all duration-300 ${scrolled ? 'bg-navy-dark bg-opacity-90 shadow-md' : 'bg-transparent'}`}>
          ...
          <button className="text-slate-lighter hover:text-teal focus:outline-none" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
            {isMobileMenuOpen ? <X size={24} /> : <Menu size={24} />}
          </button>
        </div>
        
        {isMobileMenuOpen && <div className="fixed inset-0 bg-navy-dark bg-opacity-95 flex flex-col items-center justify-center z-40">
            <div className="flex flex-col items-center space-y-14">
              {navLinks.map(link => <button key={link.id} onClick={() => handleNavClick(link.id)} className={`text-3xl font-heading transition-colors duration-300 ${activeSection === link.id ? 'text-teal' : 'text-slate-lighter hover:text-teal'}`}>
                  {link.label}
                </button>)}
            </div>
          </div>}
